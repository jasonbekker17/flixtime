          ...
          <p className="my-7 flex flex-wrap gap-2">
            {movie.genres && movie.genres.length > 0 ? (
              movie.genres.map((genre) => (
                <span
                  className="mr-2 border p-2 border-gray-600 rounded dark:border-gray-200"
                  key={genre.id}
                >
                  {genre.name}
                </span>
...
<p className='my-4  '>
  ...
  <a  target="_blank" rel="noreferrer" className="hover:underline"href={`https://www.imdb.com/title/${movie.imdb_id}`}>{movie.imdb_id}</a>
